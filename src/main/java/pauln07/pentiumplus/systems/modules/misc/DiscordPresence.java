...
public class DiscordPresence extends Module {
    ...
    private final Setting<List<String>> line2Strings = sgLine2.add(new StringListSetting.Builder()
        ...
        .defaultValue("Pentium plus", "{round({server.tps}, 1)} TPS", "Playing on {server.difficulty} difficulty.", "{server.player_count} Players online")
    ...
    public void onActivate() {
        ...
        String largeText = "Pentium plus " + PentiumPlus.VERSION;
        if (!PentiumPlus.DEV_BUILD.isEmpty()) largeText += " Dev Build: " + PentiumPlus.DEV_BUILD;
        rpc.setLargeImage("meteor_client", largeText);

        currentSmallImage = SmallImage.PaulN07;
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (Utils.canUpdate()) {
            ...
            if (!lastWasInMainMenu) {
                rpc.setDetails("Pentium plus " + (PentiumPlus.DEV_BUILD.isEmpty() ? PentiumPlus.VERSION : PentiumPlus.VERSION + " " + PentiumPlus.DEV_BUILD));
